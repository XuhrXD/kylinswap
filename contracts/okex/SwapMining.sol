...
contract SwapMining is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 allocKyiAmount; // How many KYIs
    ...
    // Only tokens in the whitelist can be mined KYI
    ...
    function getKyiReward(uint256 _lastRewardBlock) public view returns (uint256) {
    ...
    function mint(uint256 _pid) public returns (bool) {
        ...
        uint256 blockReward = getKyiReward(pool.lastRewardBlock);
        if (blockReward <= 0) {
            return false;
        }
        ...
        kyi.mint(address(this), kyiReward);
        // Increase the number of tokens in the current pool
        pool.allocKyiAmount = pool.allocKyiAmount.add(kyiReward);
    ...
    function swap(address account, address input, address output, uint256 amount) public onlyRouter returns (bool) {
        ...
        address pair = IKyiFactory(factory).pairFor(input, output);
    ...
    function takerWithdraw() public {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            ...
            if (user.quantity > 0) {
                ...
                uint256 userReward = pool.allocKyiAmount.mul(user.quantity).div(pool.quantity);
                pool.quantity = pool.quantity.sub(user.quantity);
                pool.allocKyiAmount = pool.allocKyiAmount.sub(userReward);
                user.quantity = 0;
                user.blockNumber = block.number;
        ...
        kyi.transfer(msg.sender, userSub);
    }
    ...
    function getUserReward(uint256 _pid) public view returns (uint256, uint256){
        ...
        if (user.quantity > 0) {
            uint256 blockReward = getKyiReward(pool.lastRewardBlock);
            uint256 kyiReward = blockReward.mul(pool.allocPoint).div(totalAllocPoint);
            userSub = userSub.add((pool.allocKyiAmount.add(kyiReward)).mul(user.quantity).div(pool.quantity));
        }
        //Kyi available to users, User transaction amount
    ...
    function getPoolInfo(uint256 _pid) public view returns (address, address, uint256, uint256, uint256, uint256){
        require(_pid <= poolInfo.length - 1, "SwapMining: Not find this pool");
        PoolInfo memory pool = poolInfo[_pid];
        address token0 = IKyiPair(pool.pair).token0();
        address token1 = IKyiPair(pool.pair).token1();
        uint256 kyiAmount = pool.allocKyiAmount;
        uint256 blockReward = getKyiReward(pool.lastRewardBlock);
        uint256 kyiReward = blockReward.mul(pool.allocPoint).div(totalAllocPoint);
        kyiAmount = kyiAmount.add(kyiReward);
        //token0,token1,Pool remaining reward,Total /Current transaction volume of the pool
        return (token0, token1, kyiAmount, pool.totalQuantity, pool.quantity, pool.allocPoint);
    }
    ...
    function getQuantity(address outputToken, uint256 outputAmount, address anchorToken) public view returns (uint256) {
        ...
        if (outputToken == anchorToken) {
        ...
        } else if (IKyiFactory(factory).getPair(outputToken, anchorToken) != address(0)) {
            ...
            for (uint256 index = 0; index < length; index++) {
                ...
                if (IKyiFactory(factory).getPair(outputToken, intermediate) != address(0) && IKyiFactory(factory).getPair(intermediate, anchorToken) != address(0)) {
