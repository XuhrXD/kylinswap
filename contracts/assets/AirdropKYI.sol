...
contract AirdropKYI is Ownable {
    ...
    function newAirdrop(uint256 _kyiAmount, uint256 _newPerBlock, uint256 _startBlock) public onlyOwner {
        require(block.number > endBlock && _startBlock >= endBlock, "Not finished");
        massUpdatePools();
        uint256 beforeAmount = IERC20(kyi).balanceOf(address(this));
        TransferHelper.safeTransferFrom(kyi, msg.sender, address(this), _kyiAmount);
        uint256 afterAmount = IERC20(kyi).balanceOf(address(this));
        uint256 balance = afterAmount.sub(beforeAmount);
        require(balance == _kyiAmount, "Error balance");
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 lpSupply;
        if (address(pool.lpToken) == kyi) {
            lpSupply = pool.kyiAmount;
        } else {
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            ...
            if (pendingAmount > 0) {
                safeKYITransfer(msg.sender, pendingAmount, pool.kyiAmount);
            }
        }
        if (_amount > 0) {
            ...
            if (address(pool.lpToken) == kyi) {
                pool.kyiAmount = pool.kyiAmount.add(_amount);
            }
        }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        if (_amount > 0) {
            ...
            if (address(pool.lpToken) == kyi) {
                pool.kyiAmount = pool.kyiAmount.sub(_amount);
            }
    ...
    function emergencyWithdraw(uint256 _pid) public {
        ...
        user.rewardDebt = 0;
        if (address(pool.lpToken) == kyi) {
            pool.kyiAmount = pool.kyiAmount.sub(amount);
        }
    ...
    function safeKYITransfer(address _to, uint256 _amount, uint256 _poolKYIAmount) internal {
        uint256 kyiBalance = IERC20(kyi).balanceOf(address(this));
        kyiBalance = kyiBalance.sub(_poolKYIAmount);
        if (_amount > kyiBalance) {
            IERC20(kyi).transfer(_to, kyiBalance);
        } else {
            IERC20(kyi).transfer(_to, _amount);
        }
    }
}